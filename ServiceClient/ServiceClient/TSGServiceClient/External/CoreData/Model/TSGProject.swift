...
public class TSGProject: NSManagedObject {
    ...
    public class func saveProjectInfo(dictionary:NSMutableDictionary){
        ...
        context.performBlockAndWait
            {
                let entity =  NSEntityDescription.entityForName("TSGProject",
                    inManagedObjectContext:context)
                
                let projectObj:TSGProject = NSManagedObject(entity: entity!,
                    insertIntoManagedObjectContext: context) as! TSGProject
                ...
                projectObj.setValue(TSGAPI.allProjectAPI(dictionary.valueForKey("actions")!, context: context), forKey: "requiredAPI")
    ...
    class func checkIfProjectIdExist(actionName:String)->Bool{
            ...
            var objects: [TSGProject]!
            
            context.performBlockAndWait {
                //2
                do {
                    let fetchRequest = NSFetchRequest(entityName: "TSGProject")
                    fetchRequest.predicate = NSPredicate(format: "projectID = %@", actionName)
                    fetchRequest.fetchLimit = 1
                    try objects = context.executeFetchRequest(fetchRequest) as? [TSGProject]
